...
#if !defined(_BASIC_EDGE_LOCATOR_H_)
#define _BASIC_EDGE_LOCATOR_H_
...
namespace EdgeLocator{
	...
	class BasicEdgeLocator: public AbstractEdgeLocator {
	...
	private:
		...
		std::vector<Edge> detectVerticalEdges(cv::Mat& image, float threshold, int order) {
			...
			for (int edge : edges) {
				...
				AA = (imageData[edge - m * cols + 1]
					+ imageData[edge - m * cols + 2]
					+ imageData[edge + 2]) / 3;  

				BB = (imageData[edge - 2]
					+ imageData[edge + m * cols - 2]
					+ imageData[edge + m * cols - 1]) / 3;

				// sum columns
				float SL = 0, SM = 0, SR = 0;

				for (int n = 0; n < 5; n++) {
					SL += imageData[edge - cols - 2 + n];
					SM += imageData[edge - 2 + n];
					SR += imageData[edge + cols - 2 + n];
				}
